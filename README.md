...
### 初回読み込み時の遅延について
本アプリのバックエンドは Render の無料プランを利用しているため、一定期間アクセスがないとサーバーが自動的にスリープ状態に入ります。

そのため、**サイト訪問直後の初回アクセス時に、データの読み込みに 50秒〜1分程度の時間を要する場合があります。** 一度起動すれば、その後の操作は高速に動作します。
...
- **自動更新：** 3時間おきに GitHub Actions からバックエンドの取得 API を自動で呼び出し、常に最新のニュースがDBに蓄積
